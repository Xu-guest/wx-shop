...
const urlData = hotMap.find((v) => v.type === query.type)
//设置标题
uni.setNavigationBarTitle({ title: urlData!.title })
//推荐封面图
...
//获取热门推荐数据
const getHot = async () => {
  const res = await getHotAPI(urlData!.url, { page: import.meta.env.DEV ? 33 : 1 })
...
const onScrolltolower = async () => {
  ...
  //获取新的数据
  const res = await getHotAPI(urlData!.url, {
    subTypes: currSubType.id,
